...
{
public:
	SASS::Program *Generate(const PTX::Program *program)
	{
		//TODO: Generate SASS program

		auto sassProgram = new SASS::Program();
		auto sassFunction = new SASS::Function("main_1");

		sassFunction->AddParameter(8); // t1          0x140
		sassFunction->AddParameter(8); // t1:size     0x148
		sassFunction->AddParameter(8); // $r0         0x150
		sassFunction->AddParameter(8); // $r0:size    0x158
		sassFunction->AddParameter(4); // $g          0x160

		sassFunction->SetRegisters(7);

		auto sassBlock0 = new SASS::BasicBlock("BB0");
		sassFunction->AddBasicBlock(sassBlock0);
		...
		sassProgram->AddFunction(sassFunction);
