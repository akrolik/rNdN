...
static Shape *SymbolicShapeFromType(const Type *type, const std::string& name)
{
	switch (type->m_kind)
	{
		case Type::Kind::Wildcard:
		{
		...
		{
			auto dictionaryType = static_cast<const DictionaryType *>(type);
		...
		case Type::Kind::KeyedTable:
		{
			auto tableType = static_cast<const KeyedTableType *>(type);
	...
	Utils::Logger::LogError("Unknown symbolic shape for type " + TypeUtils::TypeString(type));
}

static Shape *InitialShapeFromType(const Type *type)
{
	switch (type->m_kind)
	{
		case Type::Kind::Wildcard:
	...
	Utils::Logger::LogError("Unknown initial shape for type " + TypeUtils::TypeString(type));
}

static Shape *ShapeFromType(const Type *type, const CallExpression *call = nullptr, unsigned int tag = 0)
{
	switch (type->m_kind)
	{
		case Type::Kind::Wildcard:
	...
	Utils::Logger::LogError("Unknown default shape for type " + TypeUtils::TypeString(type));
}
