...
public:
	...
	public:
		...
		DynamicSize(const Expression *expression, unsigned int tag = 0) : Size(SizeKind), m_expression(expression), m_tag(tag) {}
...
public:
	...
	WildcardShape(const Expression *expression) : Shape(Shape::Kind::Wildcard), m_expression(expression) {}
