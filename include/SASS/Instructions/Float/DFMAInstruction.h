...
public:
	...
	DFMAInstruction(Register *destination, Register *sourceA, Composite *sourceB, Register *sourceC, Round round = Round::RN, Flags flags = Flags::None)
		: PredicatedInstruction({destination, sourceA, sourceB, sourceC}), m_destination(destination), m_sourceA(sourceA), m_sourceB(sourceB), m_sourceC(sourceC), m_round(round), m_flags(flags) {}
